...
const page = PHARMACEUTICAL_PAGE;

const whyIcons = [FlaskConical, Network, Settings2, Zap];

export function PharmaceuticalIndustryPage() {
  ...
  const SectorIcon = sectorIcons[sector.icon as keyof typeof sectorIcons] ?? Pill;

  return (
    <>
      <PageHero label="Industries" title={page.title} description={page.intro} dark />

      {/* Life sciences highlight */}
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:gap-10">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-brand-orange/20 ring-1 ring-brand-orange/30">
                <HeartPulse className="h-8 w-8 text-brand-orange" />
              </div>
              <div>
                ...
                <p className="mt-2 max-w-3xl text-lg leading-relaxed text-white/70">
                  From drug discovery and clinical trials to regulatory compliance and patient
                  care—we connect organizations with talent that advances science and saves lives.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
            {page.coverage.sectors.map((s, i) => {
              const Icon = sectorIcons[s.icon as keyof typeof sectorIcons] ?? Pill;
              return (
                <motion.div
                  key={s.id}
          ...
          <div className="grid gap-8 lg:grid-cols-12">
            <div className="flex gap-3 overflow-x-auto pb-2 lg:col-span-4 lg:flex-col lg:overflow-visible lg:pb-0">
              {page.coverage.sectors.map((item, i) => {
                const Icon = sectorIcons[item.icon as keyof typeof sectorIcons] ?? Pill;
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            <div className="flex flex-col justify-center bg-gradient-to-br from-brand-orange to-orange-700 p-8 lg:col-span-2">
              ...
              <p className="font-sans text-lg font-bold text-white">Life sciences talent</p>
            </div>
